		...
		<div class="row page-titles mx-0">
			<div class="col-sm-6 p-md-0">
				<div class="welcome-text">
					...
					<a href="#tambah" data-toggle="modal" class="btn btn-success btn-sm mt-4"><i class="fa fa-circle-plus"></i> Tambah Fasilitas Unit</a> 
				</div>
			</div>
			<div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
				<ol class="breadcrumb">
					<li class="breadcrumb-item"><a href="javascript:void(0)"></a>Unit</li>
					<li class="breadcrumb-item active"><a href="javascript:void(0)">Data Fasilitas Unit</a></li>
				</ol>
			</div>
		</div>
		<!-- row -->
			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h4 class="card-title">List Fasilitas Unit</h4>
					</div>
					<div class="card-body">
								...
								<tbody>
									...
									foreach($fasilitas as $value) { ?>
										<tr>
											...
											<td><?= $value->name_fasility ?></td>
											<td><img src="<?= base_url('assets/image/unit/fasility/' . $value->image_fasility) ?>" width="80"></td>		
											<td>
												...
												<a href="<?= base_url('Admin/hapus_fasilitas/' . $value->id) ?>" class="badge bg-danger tombol-hapus" style="color: #fff" title="Hapus"><i class="fa fa-trash"></i></a>
											</td>
										</tr>
									<?php } ?>
								</tbody>
	...
	<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?= base_url('Admin/simpan_fasilitas') ?>" name="form_tambah" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" name="name_fasility">
                    </div>
					<div class="form-group">
						<label>Foto Fasilitas <span class="text-danger">*</span></label>
						<input type="file" class="form-control" accept="image/*" id="imgInp" name="image_fasility">
						<p>maksimum 3MB</p>
	...
	<div class="modal fade" id="edit<?= $edit->id ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="<?= base_url('Admin/ubah_fasilitas') ?>" name="form_edit" method="post" enctype="multipart/form-data">
						<div class="form-group">
							<label>Pilih Unit <span class="text-danger">*</span></label>
						...
						<div class="form-group">
							<label>Fasilitas <span class="text-danger">*</span></label>
							<input type="text" class="form-control" value="<?= $edit->name_fasility ?>" name="name_fasility">
						</div>
						<div class="form-group">
							<label>Foto Fasilitas <span class="text-danger">*</span></label>
							<input type="hidden" class="form-control" value="<?= $edit->image_fasility ?>" name="old_image">
							<input type="file" class="form-control" accept="image/*" id="ImgUpload" name="image_fasility">
							<p>maksimum 3MB</p>
							<img class="mt-2 mb-3" height="150" width="150" src="<?= base_url('assets/image/unit/fasility/' . $edit->image_fasility) ?>" alt="gambar lama" />
...
<script>
	$(document).ready(function() {
		...
		$('.tombol-hapus').on('click', function(e) {
			...
			Swal.fire({
				...
				text: "Data fasilitas akan dihapus!",
				icon: 'warning',
		...
		$(function() {
			$("form[name='form_tambah']").validate({
				...
				messages: {
					...
					image_fasility: {
						required: "foto fasilitas wajib di isi"
					}
				},
	...
	<?php if ($this->session->flashdata('success_simpan')) : ?>
			Swal.fire({
				icon: 'success',
				title: 'Berhasil!',
				text: 'fasilitas unit berhasil disimpan!',
		...
		<?php elseif ($this->session->flashdata('success_ubah')) : ?>
			Swal.fire({
				icon: 'success',
				title: 'Berhasil!',
				text: 'fasilitas unit berhasil diperbarui!',
				showConfirmButton: true,
				// timer: 1500
		})
		...
		<?php elseif ($this->session->flashdata('success_hapus')) : ?>
			Swal.fire({
				icon: 'success',
				title: 'Berhasil!',
				text: 'fasilitas unit berhasil dihapus!',
				showConfirmButton: true,
				// timer: 1500
		})
		<?php endif ?>
