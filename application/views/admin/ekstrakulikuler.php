		...
		<div class="row page-titles mx-0">
			<div class="col-sm-6 p-md-0">
				<div class="welcome-text">
					...
					<a href="#tambah" data-toggle="modal" class="btn btn-success btn-sm mt-4"><i class="fa fa-circle-plus"></i> Tambah Ekstrakulikuler</a> 
				</div>
			</div>
			<div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
				<ol class="breadcrumb">
					<li class="breadcrumb-item"><a href="javascript:void(0)"></a>Unit</li>
					<li class="breadcrumb-item active"><a href="javascript:void(0)">Data Ekstrakulikuler</a></li>
				</ol>
			</div>
		</div>
		<!-- row -->
						...
						<div class="table-responsive">
							<table id="example" class="display" style="min-width: 845px">
								<thead>
									<tr>
										<th>No</th>
										<th>Nama Kegiatan Ekstrakulikuler</th>
										<th>Foto</th>
										<th>Opsi</th>
									</tr>
								</thead>
								<tbody>
									...
									foreach($ekstrakulikuler as $value) { ?>
										<tr>
											<td><?= $no++ ?></td>
											<td><?= $value->name_extra ?></td>		
											<td><img src="<?= base_url('assets/image/extracurricular/' . $value->image) ?>" width="80"></td>		
											<td>
												...
												<a href="<?= base_url('Admin/hapus_ekstrakulikuler/' . $value->id) ?>" class="badge bg-danger tombol-hapus" style="color: #fff" title="Hapus"><i class="fa fa-trash"></i></a>
											</td>
										</tr>
									<?php } ?>
								</tbody>
	...
	<div class="modal-dialog modal-xl modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?= base_url('Admin/simpan_ekstrakulikuler') ?>" name="form_tambah" method="post" enctype="multipart/form-data">
					<div class="form-group">
						<label>Nama Ekstrakulikuler <span class="text-danger">*</span></label>
						<input type="text" class="form-control" name="name_extra">
                    </div>
					<div class="form-group">
						<label>Profil Ekstrakulikuler <span class="text-danger">*</span></label>
						<textarea type="text" name="profile_extra" class="summernote"></textarea>
					</div>
					<div class="form-group">
...
<script>
		...
		$(function() {
			$("form[name='form_tambah']").validate({
				...
				messages: {
					...
					profile_extra: {
						required: "profil ekstrakulikuler wajib di isi"
					},
					image: {
	...
	<?php if ($this->session->flashdata('success_simpan')) : ?>
			Swal.fire({
				icon: 'success',
				title: 'Berhasil!',
				text: 'ekstrakulikuler berhasil disimpan!',
		...
		<?php elseif ($this->session->flashdata('success_ubah')) : ?>
			Swal.fire({
				icon: 'success',
				title: 'Berhasil!',
				text: 'ekstrakulikuler berhasil diperbarui!',
				showConfirmButton: true,
				// timer: 1500
		})
